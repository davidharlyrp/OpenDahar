...
### 2. Reset Total (Hapus `.pth` & `.json`)
*Hapus `model_geoteknik.pth` dan `data/vocab.json` sebelum menjalankan `main.py`.*
...
> [!TIP]
> **Selalu backup** file `model_geoteknik.pth` sebelum melakukan reset jika Anda ingin membandingkan hasil model lama dengan yang baru.
